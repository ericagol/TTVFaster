...
include("../test_ttv.jl")  
calc_ttvs_given_param(param::Vector) = vcat(test_ttv(5,40,20,param,WriteOutput=false,num_evals=1));
...
function calc_chisq(theta::Vector)
   ...
   for i in 1:length(ttv_pred)
     delta = ((ttv_pred[i]-ttv_ref[i]::Float64)/sigma_ttv[i]::Float64)
     chisq += delta*delta
   end
   return chisq
end
...
@time chisq_list = typeof(chisq)[calc_chisq(param_pert) for i in 1:10000]; 
...
#println("# Ignore this (jac_ttvs): ", hash(jac_list))

#@time jac = ForwardDiff.jacobian(log_likelihood,param_ref, cache=my_cache); 
#@time jac_list = typeof(jac)[ForwardDiff.jacobian(calc_chisq,param_pert, cache=my_cache) for i in 1:100];
#println("# Ignore this (jac_chisq): ", hash(jac_list))

@time hes_func = ForwardDiff.hessian(calc_chisq); # log_likelihood);
...
TargetDist.test_forw_diff(TargetDist.param_ref);
