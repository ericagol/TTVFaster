...
class Planet(object):
  def __init__(self,mass_ratio=None,trans0=None,period=None,ecos=None,esin=None):
...
# Read in parameters from file:
    data=map(float,open('kepler62ef_planets.txt').read().split(','))
...
# Subtract the transit times to only plot the TTVs:
    pl.plot(time1,model1-time1)
    pl.plot(time2,model2-time2)
    pl.show()
...
#    jmax:  maximum j to evaluated
#   param:  contains the (mass ratio,t0,period,e*cos(omega),e*sin(omega)) for each planet
# Output:
#   model1:  Contains the transit timing model with the first set for the inner planet.
#   model2:  Contains the transit timing model with the first set for the outer planet.
# Insert the parameters of the model into a structure for each planet:
    p1 = Planet(mass_ratio=param[0],trans0=param[1],period=param[2],ecos=param[3],esin=param[4])
    p2 = Planet(mass_ratio=param[5],trans0=param[6],period=param[7],ecos=param[8],esin=param[9])
#
    twopi = 2.0*np.pi
...
# Compute the coefficients:
    f1,f2=ttv_succinct(jmax+1,alpha)  # I need to compute coefficients one higher than jmax
...
# Compute TTVs for inner planet (equation 33):
    for j in range(1,jmax+1):
      ...
      ttv1 += f1[  j,1]*(p1.ecos*np.sin((j-1)*lam11-j*lam21)+p1.esin*np.cos((j-1)*lam11-j*lam21))
      ttv1 += f1[  j,2]*(p1.ecos*np.sin((j+1)*lam11-j*lam21)-p1.esin*np.cos((j+1)*lam11-j*lam21))
      ttv1 += f1[j-1,3]*(p2.ecos*np.sin((j-1)*lam11-j*lam21)+p2.esin*np.cos((j-1)*lam11-j*lam21))
      ttv1 += f1[j+1,4]*(p2.ecos*np.sin((j+1)*lam11-j*lam21)-p2.esin*np.cos((j+1)*lam11-j*lam21))
...
# Compute TTVs for outer planet (equation 33):
    for j in range(1,jmax+1):
      ...
      ttv2 += f2[  j,1]*(p2.ecos*np.sin(j*lam12-(j+1)*lam22)+p2.esin*np.cos(j*lam12-(j+1)*lam22))
      ttv2 += f2[  j,2]*(p2.ecos*np.sin(j*lam12-(j-1)*lam22)-p2.esin*np.cos(j*lam12-(j-1)*lam22))
      ttv2 += f2[j+1,3]*(p1.ecos*np.sin(j*lam12-(j+1)*lam22)+p1.esin*np.cos(j*lam12-(j+1)*lam22))
      ttv2 += f2[j-1,4]*(p1.ecos*np.sin(j*lam12-(j-1)*lam22)-p1.esin*np.cos(j*lam12-(j-1)*lam22))
...
def u(gamma,c1,c2):
    return ((3.0+gamma**2)*c1+2.0*gamma*c2)/gamma**2/(1.0-gamma**2)

def v(zeta,d1,d2,m):
# m=+/-1
    return ((m*(1.0-zeta**2)+6.0*zeta)*d1+(2.0+zeta**2)*d2)/(zeta*(1.0-zeta**2)*(zeta+m)*(zeta+2.0*m))
...
# Inner planet coefficients, in order k=0,-1,1,-2,2 (see Table 1):
      ...
      c1=alpha*j*(A_j00*np.array([1,-j,j,j,-j])-.5*A_j01*np.array([0,0,0,1,1])-.5*A_j10*np.array([0,1,1,0,0])-alpha*dj1*np.array([1,-1.5,.5,2,0]))
      c2=alpha*  (A_j10*np.array([1,-j,j,j,-j])-.5*A_j11*np.array([0,0,0,1,1])-.5*A_j20*np.array([0,1,1,0,0])-alpha*dj1*np.array([1,-1, 1,2,0]))
      if j >= 2:
         for k in range(5): f1[j,k]=u(gamma[k],c1[k],c2[k])
      else:
         if j == 0:
            f1[j,3]=u(gamma[3],c1[3],c2[3])
         else:
            for k in range(4): f1[j,k]=u(gamma[k],c1[k],c2[k])

# Add in the k=\pm 1 coefficients (note that d1 & d2 are the same as c1 & c2 for k=0):
      if j >= 1: f1[j,1]=f1[j,1]+v(beta,c1[0],c2[0],-1)
      if j >= 1: f1[j,2]=f1[j,2]+v(beta,c1[0],c2[0], 1)
# Now for the outer planet:
# Outer planet coefficients, in order k=0,-2,2,-1,1 (see Table 1):
      gamma = kappa+np.array([0,-1,1,-1.0/alpha**1.5,1.0/alpha**1.5])
      c1=-j*(A_j00*np.array([1,j,-j,-j,j])-.5*A_j01*np.array([0,1,1,0,0])-.5*A_j10*np.array([0,0,0,1,1])-dj1/alpha**2*np.array([1,.5,-1.5,0,2]))
      c2=   (A_j01*np.array([1,j,-j,-j,j])-.5*A_j11*np.array([0,0,0,1,1])-.5*A_j02*np.array([0,1,1,0,0])-dj1/alpha**2*np.array([1,1,-1,0,2]))
      if j >= 2:
         for k in range(4): f2[j,k]=u(gamma[k],c1[k],c2[k])
      else:
         if j == 1:
            for k in range(3): f2[j,k]=u(gamma[k],c1[k],c2[k])
      f2[j,4]=u(gamma[4],c1[4],c2[4])
# Add in the k=\pm 2 coefficients (note that d1 & d2 are the same as c1 & c2 for k=0):
      if j >= 1: f2[j,1]=f2[j,1]+v(kappa,c1[0],c2[0],-1)
      if j >= 1: f2[j,2]=f2[j,2]+v(kappa,c1[0],c2[0], 1)
# That's it!
#from laplace_wisdom import laplace_wisdom
    return f1,f2
...
# Recurrence relations (derived on 9/9/14; also see Brouwer & Clemence):
      jd = float(j0)
      b[j0,0]=(-(2.0*jd+3.0)*b[j0+2,0]+(1.0+alpha*alpha)/alpha*2.0*(jd+1.0)*b[j0+1,0])/(2.0*jd+1.0)
# Recurrence for first derivative:
      b[j0,1]=b[j0+2,1]-(2.0*alpha*(jd+1.0)*b[j0+1,0]-jd*b[j0,0]-(jd+2.0)*b[j0+2,0])/alpha
# Recurrence for second derivative:
      b[j0,2]=b[j0+2,2]-2.0*(jd+1.0)*b[j0+1,1]-(jd*b[j0,0]+(jd+2.0)*b[j0+2,0])/alpha/alpha+\
...
#   by series summation */
    ...
    if j <= i:  #/* compute first term in sum */
       factor4 = 1.0
       for k in range(j): factor4 = factor4*float(i - k)
       sum = factor4
       q0 = 0
    else:
       q0 = (j + 1 - i)/2
...
#  /* compute factors for terms in sum */
    ...
    factor3 = float(i) + 1.0
    q=1
    for q in range(1,q0):   #/* no contribution for q = 0 */
        factor1 = factor1*(s + float(q))
        factor2 = factor2*(s + float(i + q))
        factor3 = factor3*(float(i+q+1))
...
#  /* sum series */
    ...
    while (term*factor4) > LAPLACE_EPS:
         factor4 = 1.0
         for k in range(j): factor4 = factor4*(2.0*float(q) + float(i - k))
         sum += term * factor4
         factor1 += 1.0
         factor2 += 1.0
         factor3 += 1.0
...
#  /* fix coefficient */
    ...
    for k in range(i): sum = sum*(s + float(k))/(float(k)+1.0)

    if q0 <= 0:
       sum = sum*2.0 * a**i
    else:
       sum = sum*2.0 * a**(2*q0 + i - 2)
